 ...
 * driver functions
 **********************************************************/
static ssize_t pn54x_dev_read(struct file *filp, char __user *buf, size_t count,
                              loff_t *offset) {
  ...
  if (!gpio_get_value(pn54x_dev->irq_gpio)) {
    ...
    while (1) {
      ...
      ret =
          wait_event_interruptible(pn54x_dev->read_wq, !pn54x_dev->irq_enabled);
...
#endif
{
  ...
  pn54x_dev->irq_enabled = true;
  ret = request_irq(client->irq, pn54x_dev_irq_handler, IRQF_TRIGGER_HIGH,
                    client->name, pn54x_dev);
  if (ret) {
